...
public class UIManager : MonoBehaviour
{
		...
		get
		{
			if (s_Instance == null)
			{
				...
				if (s_Instance == null && Application.isEditor)
					if ( Wizards.Utils.DEBUG ) Debug.LogError("Could not locate a UIManager object. You have to have exactly one UIManager in the scene.");
			}
	...
	*/
	...
	void Awake()
	{
		...
		if (s_Instance != null)
		{
			if ( Wizards.Utils.DEBUG ) Debug.LogError("You can only have one instance of this singleton object in existence.");
		}
		else
			s_Instance = this;
		...
		*/
		...
		switch (pointerType)
		{
			...
			case POINTER_TYPE.TOUCHPAD_AND_RAY:
				...
				break;
			default:
				if ( Wizards.Utils.DEBUG ) Debug.LogError("ERROR: Invalid pointer type selected!");
				break;
		}
	}
...
// If we have a mouse under any possible circumstances, poll it:
...
#if (UNITY_3_0 || UNITY_3_1 || UNITY_3_2 || UNITY_3_3 || UNITY_3_4 || UNITY_3_5 || UNITY_3_6 || UNITY_3_7 || UNITY_3_8 || UNITY_3_9 || UNITY_4_1 || UNITY_4_2 || UNITY_4_3)
		Touch touch;
#else
		Touch touch;
#endif
	...
	{
		//if ( Wizards.Utils.DEBUG ) Debug.LogWarning("The UIManager encountered a collider on object \"" + obj.name + "\" that does not not contain a UIObject or derivative component.  Please double-check that this object has the correct layer and components assigned.");
	}
}
