	...
	public override void Start()
	{
		...
		if(Application.isPlaying)
		{
			...
			for (int i = 0; i < filledLayers.Length; ++i)
			{
				...
				{
					if ( Wizards.Utils.DEBUG ) Debug.LogError("A null layer sprite was encountered on control \"" + name + "\". Please fill in the layer reference, or remove the empty element.");
					continue;
				}
			...
			else
			{
				if (manager != null)
				{
					...
					if ( Wizards.Utils.DEBUG ) Debug.LogError("Sprite on object \"" + name + "\" not assigned to a SpriteManager!");
			}
