...
public class SpriteFont
{
	...
	int FindField(string label, string[] fields, int pos, bool logError)
	{
		...
		if (logError)
		{
			if ( Wizards.Utils.DEBUG ) Debug.LogError("Missing \"" + label + "\" field in font definition file \"" + fontDef.name + "\". Please check the file or re-create it.");
			return pos;
		}
		else
			return -1;
	...
	void CommonParser(string line)
	{
		...
		if (int.Parse(vals[index + 1]) > 1)
			if ( Wizards.Utils.DEBUG ) Debug.LogError("Multiple pages/textures detected for font \"" + face + "\". only one font atlas is supported.");
	}
	...
	void CharCountParser(string line)
	{
		...
		if (vals.Length < 2)
		{
			if ( Wizards.Utils.DEBUG ) Debug.LogError("Malformed \"chars count\" line in font definition file \"" + fontDef.name + "\". Please check the file or re-create it.");
			return;
		}
