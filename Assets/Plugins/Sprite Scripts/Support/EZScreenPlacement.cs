...
public class EZScreenPlacement : MonoBehaviour, IUseCamera
{
	...
	public void PositionOnScreen()
	{
		...
		if (renderCamera == null)
		{
			if ( Wizards.Utils.DEBUG ) Debug.LogError("Render camera not yet assigned to EZScreenPlacement component of \"" + name + "\" when attempting to call PositionOnScreen()");
			return;
		}
...
public class EZScreenPlacementMirror
{
	...
	public virtual bool Validate(EZScreenPlacement sp)
	{
		...
		if (sp.relativeObject != null)
		{
			...
			{
				if ( Wizards.Utils.DEBUG ) Debug.LogError("ERROR: The Relative Object you recently assigned on \"" + sp.name + "\" which points to \"" + sp.relativeObject.name + "\" would create a circular dependency.  Please check your placement dependencies to resolve this.");
				sp.relativeObject = null;
			}
		}
