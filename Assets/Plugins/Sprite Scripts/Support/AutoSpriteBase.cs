...
public abstract class AutoSpriteBase : SpriteBase, ISpriteAggregator
{
	...
	public override Vector2 GetDefaultPixelSize(PathFromGUIDDelegate guid2Path, AssetLoaderDelegate loader)
	{
		...
		if(f == null)
		{
			if ( Wizards.Utils.DEBUG ) Debug.LogWarning("Sprite \"" + name + "\" does not seem to have been built to an atlas yet.");
			return Vector2.zero;
		}
	...
	public void PlayAnim(int index, int frame)
	{
		...
		{
			if ( Wizards.Utils.DEBUG ) Debug.LogError("ERROR: Animation index " + index + " is out of bounds!");
			return;
		}
	...
	public void PlayAnim(string name, int frame)
	{
		...
		if ( Wizards.Utils.DEBUG ) Debug.LogError("ERROR: Animation \"" + name + "\" not found!");
	}
