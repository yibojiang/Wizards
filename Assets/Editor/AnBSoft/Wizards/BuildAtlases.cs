...
public class BuildAtlases : ScriptableWizard
{
...
#if UNITY_IPHONE && !(UNITY_3_0 || UNITY_3_1 || UNITY_3_2 || UNITY_3_3 || UNITY_3_4 || UNITY_3_5 || UNITY_3_6 || UNITY_3_7 || UNITY_3_8 || UNITY_3_9 || UNITY_4_1 || UNITY_4_2 || UNITY_4_3)
	...
	void OnWizardCreate()
	{
		...
		for (int i = 0; i < matSpriteMap.Count; ++i)
		{
			...
			{
				if ( Wizards.Utils.DEBUG ) Debug.LogWarning("Warning: No textures found for material \"" + texList.material.name + "\".");
				--numAtlasesBuilt;
			}
			else
				BuildAtlas(texList);
		...
		if ( Wizards.Utils.DEBUG ) Debug.Log(numAtlasesBuilt + " atlases generated.");
	...
	void ScanProjectFolder(ArrayList sprites)
	{
		...
		while (stack.Count > 0)
		{
			...
			{
				// Error
				if ( Wizards.Utils.DEBUG ) Debug.LogError("Could not access folder: \"" + dir + "\"");
			}
		}
	}
		...
		for (int i = 0; i < sprites.Count; ++i)
		{
			...
			if (sprite.spriteMesh == null)
			{
				...
				if (sprite.managed)
				{
					...
					{
						mat = sprite.manager.GetComponent<Renderer>().sharedMaterial;
					}
				...
				{	 // as this is probably a prefab and that's why it
					// doesn't have a mesh:
					mat = sprite.GetComponent<Renderer>().sharedMaterial;
				}
			}
		...
		for (int i = 0; i < msMap.sprites.Count; ++i)
		{
			...
			for (int j = 0; j < spr.SourceTextures.Length; ++j)
			{
				...
				else
				{
					...
					if ( Wizards.Utils.DEBUG ) Debug.LogWarning("Null texture reference detected on sprite \"" + ((SpriteRoot)spr).name + "\"! Please verify that this was intentional.");
#endif
				}
			}
		}
...
// 		atlas = (Texture2D) AssetDatabase.LoadAssetAtPath(projectRelativePath, typeof(Texture2D));
		   ...
		   texList.uvs[0].height * atlas.height != ((Texture2D)texList.trimmedTextures[0]).height)
		{
			...
			if ( Wizards.Utils.DEBUG ) Debug.LogWarning("WARNING: Textures were resized to fit onto atlas \"" + atlas.name + "\"!  To resolve this, assign some of your sprites a different material, or if possible, use a larger maximum texture size.");
		}
		...
		if ( Wizards.Utils.DEBUG ) Debug.Log("Finished building atlas \"" + texList.material.name + "\"...");
	}
	...
	void SetFrameInfo(MaterialSpriteList msMap, AtlasTextureList texList)
	{
		...
		for (int i = 0; i < msMap.sprites.Count; ++i)
		{
			...
			for (int j = 0; j < spAg.SpriteFrames.Length; ++j)
			{
				...
				if (index < 0)
				{
					if ( Wizards.Utils.DEBUG ) Debug.LogError("Error assigning frame info: frame " + j + " on sprite \"" + ((SpriteRoot)spAg).name + "\" not found in the texture list!");
					continue;
				}
