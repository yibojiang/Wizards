...
public class Generic3DGUI : MonoBehaviour
{
...
void FixedUpdate(){
    ...
    if(ObjectGlow==true){
        if(CurrentOpacity < 0.5f && CurrentGlowDirection == 1){
            GetComponent<Renderer>().material.color = new Color(0.5f,0.5f,0.5f,CurrentOpacity);
...
void fncInitButton(){
    ...
    case SelectGUI_ObjectType.Button:
        GetComponent<Renderer>().material.mainTexture=TextureMouseNormal;
        if(ButtonEnabled==false){
            GetComponent<Renderer>().material.color=ButtonDisabledColor;
        }
        break;
...
switch(strInternalCommand)
    ...
    case "ToggleFreeze":
      if(Frozen==false){
            GetComponent<Renderer>().material.color = new Color(0.5f,0.5f,0.5f,0.1f);
        }else{
...
if(strFadeType=="FadeOut"){
    fltFadeOpacity=GetComponent<Renderer>().material.color.a;
    ObjectGlow=false;
}
while (intRunFade!=0)
        {
            switch (strFadeType)
            {
                case "FadeIn":
                    if(fltFadeOpacity <= fltMaxOpacity){
                        GetComponent<Renderer>().material.color = new Color(0.5f,0.5f,0.5f,fltFadeOpacity);
...
if(strFadeType=="FadeIn"){ 
    GetComponent<Renderer>().material.color = new Color(0.5f,0.5f,0.5f,fltMaxOpacity);
}
    if(strFadeType=="FadeOut"){
        GetComponent<Renderer>().material.color = new Color(0.5f,0.5f,0.5f,0.0f);
        GetComponent<Renderer>().enabled=false;
    }
}
...
void fncSetButtonState (string strState) {
    ...
    case "Over":
        ...
        if(TextureMouseOver!=null){
            GetComponent<Renderer>().material.mainTexture=TextureMouseOver;
        }else{
            GetComponent<Renderer>().material.color = new Color(0.6f,0.6f,0.6f,0.6f);  
        }
      break;
    case "Selected":

        if(TextureMouseSelected!=null){
            GetComponent<Renderer>().material.mainTexture=TextureMouseSelected;
            GetComponent<Renderer>().material.color = new Color(0.5f,0.5f,0.5f,0.5f);
        }
      break;
    case "Active":
            GetComponent<Renderer>().material.color = new Color(0.8f,0.8f,0.8f,0.5f);
      break;
    default:
      //No Action
      break;
...
if(Frozen==false){
    ...
    ContextHelpText.GetComponent<Renderer>().enabled = true;
    }
    switch(m_SelectGUI_ObjectType)
    {
    case SelectGUI_ObjectType.Button:
...
if(Frozen==false){
    MouseIsOver = false;
    if(ContextHelpText != null){
    ContextHelpText.GetComponent<Renderer>().enabled = false;
    }
    switch(m_SelectGUI_ObjectType)
    {
    case SelectGUI_ObjectType.Button:
