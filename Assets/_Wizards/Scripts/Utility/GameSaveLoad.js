...
{
	var lm:LevelManager=LevelManager.Instance();
	var offSet : int = 0;
	var destLevel : int = 0;
	
	if ( Wizards.Utils.DEBUG ) Debug.Log("LoadXML : Attempting : " + _path);
	var _data : String = "";
	if ( FileExists(_path) == true ){
		_data=LoadXMLFile(_path);
	}
	else
	{
...
function SaveLevelDataSelection(_fullnameandpath : String, _sourceStart : int, _sourceEnd : int)
{
    ...
    var _data:String = SerializeObject(myData);
    //_data = (magic.GetComponent("Magic") as Magic).EncryptThis(_data);
    // This is the final resulting XML from the serialization process
   
    CreateFile(_data,_fullnameandpath);
...
static function CreateFile(_data:String, _fullnameandpath : String)
{
...
static function FileExists(_fullnameandpath : String) : boolean
{
...
static function LoadXMLFile(_fullnameandpath : String) : String
{
	var r : StreamReader = File.OpenText(_fullnameandpath);
